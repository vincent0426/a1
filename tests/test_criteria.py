...
from a1 import (
    Agent, Tool, Strategy,
    QualitativeCriteria, QuantitativeCriteria,
    LLM, LLMOutput
)
...
class TestQualitativeCriteria:
    ...
    def test_single_sample_pass(self, mock_llm):
        ...
        criteria = QualitativeCriteria(
            expression="Code is readable and well-structured",
            llm=mock_llm,
            num_samples=1
        )
    ...
    def test_single_sample_fail(self, mock_llm):
        ...
        criteria = QualitativeCriteria(
            llm=mock_llm,
            expression="This requirement should fail"
        )
...
async def test_qualitative_criteria_real_llm():
    ...
    criteria = QualitativeCriteria(
        expression="The code is syntactically valid Python",
        llm=llm,
        num_samples=1
    )
