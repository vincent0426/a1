    ...
    async def generate(
        ...
        if len(all_tools) > self.max_tools:
            ...
            async def filter_tools():
                ...
                try:
                    ...
                    tool_indices = sorted(range(len(similarities)), key=lambda i: similarities[i], reverse=True)[
                        : self.max_tools
                    ]
        ...
        async def reduce_tool_schemas():
            ...
            if tools_with_large_enums:
                logger.info(
                    f"Reduced enums in {len(tools_with_large_enums)} tool schemas: {', '.join(tools_with_large_enums[:5])}"
                )
    ...
    def verify(self, code: str | tuple[str, str], agent: Agent) -> tuple[bool, str | None]:
        ...
        for prerequisite, dependent in self.rules:
            ...
            if dependent_positions and not prereq_positions:
                return (
                    False,
                    f"Tool '{dependent}' requires '{prerequisite}' to be called first, but '{prerequisite}' was never called",
                )
            ...
            if prereq_positions and dependent_positions:
                ...
                if earliest_dependent < earliest_prereq:
                    return (
                        False,
                        f"Tool '{dependent}' (position {earliest_dependent}) was called before '{prerequisite}' (position {earliest_prereq}), violating ordering constraint",
                    )
