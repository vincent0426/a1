...
class LLM:
    """
    LLM tool that can call language models with function calling support.
    ...
    def __init__(
        ...
        # Create the tool lazily via property
        self._tool = None
            ...
            while tool_call_count < max_tool_calls:
                ...
                if target_output_schema and not api_tools:
                    ...
                    try:
                        ...
                        schema_dict = reduce_large_enums(
                            schema_dict, context_text=content, runtime=runtime, threshold=100, target_size=100
                        )
                        ...
                        response_format = prepare_response_format(
                            schema_dict, name=target_output_schema.__name__, strict=True
                        )
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        logger.warning(
                            f"Attempt {attempt + 1}/{max_retries} failed: {type(e).__name__}: {str(e)[:100]}"
                        )
                                                ...
                                                if len(result_fields) == 1:
                                                    ...
                                                    if len(target_fields) == 1:
                                                        ...
                                                        if isinstance(extracted_value, dict):
                                                            ...
                                                            # Can't convert
                                                            target_field = list(target_fields)[0]
                                                            return target_output_schema(
                                                                **{target_field: extracted_value}
                                                            )
                                                else:
