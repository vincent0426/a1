...
def compute_block_cost(
    block: BasicBlock,
    loop_depth: int = 0,
    tool_name_to_cost: Optional[Dict[str, float]] = None
) -> float:
    """
            ...
            if next_bid not in visited:
                if idx == 0:
                    ...
                    successor_cost = traverse_cfg_for_cost(
                        blocks, next_block, tool_name_to_cost,
                        loop_depth, visited.copy(), loop_headers
                    )
                    ...
                    successor_cost = traverse_cfg_for_cost(
                        blocks, next_block, tool_name_to_cost,
                        next_depth, visited.copy(), loop_headers
                    )
